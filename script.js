...
//for displaying the amount movements
const displayMovements = function (acc, sort) {
  //sorting in asc order if sort = true
  const movs = sort
  ...
  movs.forEach(function (mov, i) {
    ...
    const movDate = formatDates(date, acc.locale);
...
//to display current balance in the label
const calcDisplayBalance = function (acc) {
  acc.balance = acc.movements.reduce((acc, cur) => acc + cur, 0);
  labelBalance.textContent = formatCurr(acc.balance, acc.locale, acc.currency);
};

//to display the account stats at the bottom
const calcDisplayStats = function (acc) {
  ...
  //interests calculated on deposits, then only the ones with >=1 int will be summed up
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    //When time left is 0 secs, stop the timer and logout the user
    if (time === 0) {
      clearInterval(timer);
...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    ...
    //automatically setting dates as per user's locale
    const now = new Date();
    const options = {
      hour: 'numeric',
      minute: 'numeric',
